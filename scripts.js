...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function loadArticles(page) {
        ...
        pageArticles.forEach(article => {
            ...
            articleElement.innerHTML = `
                <h3><a href="#article" onclick="showArticle('${article.title}')">${article.title}</a></h3>
                <p>${article.content.substring(0, 100)}...</p>
                <div>作者：www.人情世故.com　·　2024年3月</div>
            `;
    ...
    window.showArticle = function(title) {
        const article = mockArticles.find(a => a.title === title);
        if (!article) return;
        
        const container = document.getElementById('articles-container');
        
        container.innerHTML = `
            <a href="#" onclick="loadArticles(${currentPage}); return false;">返回首页</a>
            ...
            <div class="article-item">
                <h2>相关文章</h2>
                <ul>
                    ${mockArticles.filter(a => a.title !== title).slice(0, 3).map(a => `
                        <li><a href="#article" onclick="showArticle('${a.title}')">${a.title}</a></li>
                    `).join('')}
                </ul>
            </div>
        `;
